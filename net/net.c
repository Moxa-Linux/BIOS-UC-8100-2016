...
#define ETHLOOP_LEN		1024
#define ETHLOOP_HEAD_SIZE	14

uchar *net_tx_packet;
uchar ether_packet[ETHLOOP_LEN + ETHLOOP_HEAD_SIZE];
uchar test_ether[6];

/**********************************************************************/
static void eth_loop_handler(uchar *pkt, unsigned dest, struct in_addr sip, unsigned src, 
	unsigned len)
{
	int i,flag = 0;
#ifdef ETH_LOOP_DEBUG
	for(i = ETHLOOP_HEAD_SIZE; i < (ETHLOOP_LEN + ETHLOOP_HEAD_SIZE); i++) {
		if(i == ETHLOOP_HEAD_SIZE)
			printf ("ETH Loopback: TX data = \n");

		if((((i - ETHLOOP_HEAD_SIZE) % 16) == 0) && ( i > 0))
				printf ("\n");
	...
	for(i = ETHLOOP_HEAD_SIZE ; i < (ETHLOOP_LEN + ETHLOOP_HEAD_SIZE); i++) {
		if (ether_packet[i] != pkt[i]) {
			printf ("\nETH Loopback test error:%02x(TX[0x%x]) != %02x(RX[0x%x]) \n",ether_packet[i], i, pkt[i], i);
	...
	if(flag == 0)
		net_set_state(NETLOOP_SUCCESS); /* we did not get the reply */

}

int eth_loop_send(void)
{
	int i, k;
	uchar *pkt;
	uchar macdec[6];
	...
	for(i = 0; i < ETHLOOP_LEN; i++) {
		k = (i % 256);
		ether_packet[ETHLOOP_HEAD_SIZE + i] = k;
	}
	...
	pkt = (uchar *)ether_packet;

	pkt += net_set_ether(pkt, macdec, PROT_TEST); /*set our MAC address as destination address */
	(void) eth_send(ether_packet, ETHLOOP_LEN + ETHLOOP_HEAD_SIZE);

	printf ("Send OK!\n");
	return 1;       /* waiting */
}

void eth_loop_timeout (void)
{
	eth_halt();
	printf ("time out\n");
	net_set_state(NETLOOP_FAIL);    /* we did not get the reply */
...
void eth_loop_start(void)
{
	printf ("Using %s device\n", eth_get_name());
	net_set_timeout_handler(1000UL, eth_loop_timeout);
	net_set_udp_handler(eth_loop_handler);
	eth_loop_send();
}
...
void net_process_received_packet(uchar *in_packet, int len)
{
	...
	case PROT_TEST:
		src_ip.s_addr = 0;
		printf("Got ethernet loopback data size, size = %d\n", net_rx_packet_len - 2);

          /*
           *      Ethernet header OK.  Pass the packet to the current handler.
           */
