...
//R00   05/17/2016   SHA   Add internal SRAM sections with cache enabled to early MMU settings.
...
void mmu_setup_all(void){
	...
	val &= ~ARMV7_DOMAIN_MASK;
	val |= ARMV7_DOMAIN_CLIENT;

	asm volatile("mcr p15, 0, %0, c3, c0, 0 @ set DACR" : : "r" (val) : "cc");
	isb();

	/* and enable the mmu */
	asm volatile("mrc p15, 0, %0, c1, c0, 0 @ get CR"
			: "=r" (val) : : "cc"); /* get control reg. */
