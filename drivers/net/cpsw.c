...
//mac_control = 0x21 for switch muilt chip mode. __raw_writel(mac_control, &slave->sliver->mac_control);
...
int _cpsw_register_bak(struct cpsw_priv *priv, int idx)
{
	struct cpsw_slave       *slave;
	struct cpsw_platform_data *data = &priv->data;
	void                    *regs = (void *)data->cpsw_base;
