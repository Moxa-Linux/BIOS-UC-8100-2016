...
static struct musb *host[2];
static struct usb_hcd hcd[2];
			...
			void *buffer, int len, struct devrequest *setup)
{
	...
	if (!dev->parent)
		dev->speed = ((struct musb *)(((struct usb_hcd *)(dev->controller))->hcd_priv))->host_speed;

	return submit_urb(((struct usb_hcd *)(dev->controller)),urb);
}
...
int usb_lowlevel_init(int index, void **controller)
{
	...
	host[index]->host_speed = (musb_readb(mbase, MUSB_POWER) & MUSB_POWER_HSMODE) ?
			USB_SPEED_HIGH :
			(musb_readb(mbase, MUSB_DEVCTL) & MUSB_DEVCTL_FSDEV) ?
			USB_SPEED_FULL : USB_SPEED_LOW;

	host[index]->is_active = 1;
	hcd[index].hcd_priv = host[index];
	*controller = &hcd[index];
