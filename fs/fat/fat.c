...
signed long long do_fat_get_file_size (const char *filename)
{
	char fnamecopy[2048];
	boot_sector bs;
	volume_info volinfo;
		...
		for (i = 0; i < DIRENTSPERBLOCK; i++) {
			...
			if (dentptr->attr & ATTR_VOLUME) {
				...
				if ((dentptr->attr & ATTR_VFAT) == ATTR_VFAT &&
					...
					if (strcmp (l_name, filename) == 0) {
						ret = 1;
						filesize = (signed long long)FAT2CPU32(dentptr->size);
						goto exit;
					} else {
						dentptr++;
						continue;
